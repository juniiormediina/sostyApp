...
export const User = () => {
  ...
  const getUserData = () =>{
    // AsyncStorage.getItem('userData')
      AsyncStorage.getItem('userData').then(res => {
        ...
        const finalUrl = url + '?userID=' + JSON.parse(res).user.userID + '&userType='+ JSON.parse(res).user.userType
        fetch(finalUrl,requestOptions).then(res =>res.status == 200 ? res.json().then(data=> {
  ...
  // const getUserInfo = () =>{
  //   getUserData();
  //   setuserId(userDataStorage.user.userId)
  //   setuserType(userDataStorage.user.userType)
  //   const finalUrl = url + '?userID=' + userId + '&userType='+ userType
  //   console.log(finalUrl)
  // }
    ...
    <StyledView className="w-full h-full flex justify-around items-center">
      <StyledScroll className="w-full">
        <StyledView className="w-full h-full flex justify-around items-center ">
          ...
          <StyledView className="flex justify-center items-center">
            <StyledText className="text-base w-80">Fecha de nacimiento</StyledText>
            <StyledTextInput
          ...
          <StyledView className="flex justify-center items-center">
            <StyledText className="text-base w-80">Tipo de documento</StyledText>
            <StyledTextInput
              placeholder="CC"
              value={docType}
              className="w-80  border-2 border-gray-300 rounded-lg mb-9"
          ...
          <StyledView className="flex justify-center items-center">
            <StyledText className="text-base w-80">Numero de documento</StyledText>
            <StyledTextInput
